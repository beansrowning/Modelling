...
require(plotly)
load("mlruns.dat")
base <- measles_land$pd92
load("sense_and_sensitivity.dat")
base$z2 <- measles_land$pd2_mod1$z
source("../Current/Parameterized/plot.R")
threedPlot(measles_land$run_2_mod2,
           variables = c("max"),
           cutoff = c(365, 730, 1095),
           xlab ="Population Size", 
           ylab ="MMR Vaccination Rate",
           zlab = "Outbreak Length (days)",
           title = "Measles Land : 92% Seroprevalence (older cases)")
measles_land$p2_mod2 <- plot
measles_land$pd2_mod2 <- plot_dat
