...
\path[->] (S1)   edge node {$\beta I_{1}$}                   (E1) % Young Compartment
                 edge node {$\theta$}                        (S2)
          (E1)   edge node {$\phi$}                          (I1)
                 edge node {$\theta$}                        (E2)
...
Where:
\par
 $\beta_{n} = $ Rate of contact between infectious and susceptible persons \par
 $\phi = $ Rate of onset of infectiousness subsequent to being infected \par
 $\gamma = $ Rate of recovery from measles from infectious period \par
 $\alpha = $ Crude birth rate \par
 $\nu = $ Effective vaccination rate at birth \par
